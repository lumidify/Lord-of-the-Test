minetest.register_craftitem("lottgear:arrow_fire", {
	description = "Fire Arrow",
	inventory_image = "throwing_arrow_fire.png",
})

minetest.register_node("lottgear:arrow_fire_box", {
	drawtype = "nodebox",
	node_box = {
		type = "fixed",
		fixed = {
...
local THROWING_ARROW_ENTITY={
	...
	textures = {"lottgear:arrow_fire_box"},
	lastpos={},
	...
	if self.timer>0.2 then
		...
		for k, obj in pairs(objs) do
			if obj:get_luaentity() ~= nil then
				if obj:get_luaentity().name ~= "lottgear:arrow_fire_entity" and obj:get_luaentity().name ~= "__builtin:item" then
					local damage = 5
...
minetest.register_entity("lottgear:arrow_fire_entity", THROWING_ARROW_ENTITY)

minetest.register_craft({
	output = 'lottgear:arrow_fire 1',
	recipe = {
