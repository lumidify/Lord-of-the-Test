...
local throwing_register_axe = function(axe, desc, damage, craft1, craft2)
	local axe_entity = "lottgear:" .. axe .. "_entity"
	local throwing_throw_axe = function(itemstack, player)
	...
	minetest.register_craftitem("lottgear:axe_" .. axe, {
		description = desc .. " Throwing Axe",
	    on_use = function(itemstack, user, pointed_thing)
	        throwing_throw_axe(item, user, pointed_thing)
	        if not creative then
	    		itemstack:take_item()
	        end
	        return itemstack
	    end,
		inventory_image = "throwing_axe_" .. axe .. "_inv.png",
	})

	minetest.register_node("lottgear:" .. axe .. "_axe_box", {
		drawtype = "nodebox",
	    node_box = {
			type = "fixed",
			fixed = {
	    ...
	    tiles = {
			"throwing_axe_top.png",
			"throwing_axe_top.png",
			"throwing_axe_back.png",
			"throwing_axe_" .. axe .. "_front.png",
			"throwing_axe_" .. axe .. ".png",
			"throwing_axe_" .. axe .. "2.png"
		},
	...
	local aep = {
		...
		textures = {"lottgear:" .. axe .. "_axe_box"},
		lastpos = {},
				...
				if obj:get_luaentity() ~= nil then
					if obj:get_luaentity().name ~= axe_entity and obj:get_luaentity().name ~= "__builtin:item" then
	                    ...
	                    if math.random(1, 4) ~= 3 and not creative then
	                        minetest.add_item(self.lastpos, 'lottgear:axe_' .. axe)
	                    end
					end
				else
					obj:punch(self.player, 1.0, {
...
throwing_register_axe("dwarf", "Dwarvern", 12, "lottores:mithril_ingot", "default:steel_ingot")
throwing_register_axe("elf", "Elven", 10, "lottores:galvorn_ingot", "default:steel_ingot")
throwing_register_axe("steel", "Steel", 5, "default:steel_ingot")
throwing_register_axe("galvorn", "Galvorn", 7, "lottores:galvorn_ingot")
