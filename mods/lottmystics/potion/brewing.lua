potion.make_pipe = function( pipes, horizontal )
   local result = {};
...
potion.brew_recipes = { cooking = { input_size = 1, output_size = 1 } }
function potion.register_recipe_type(typename, origdata)
	...
	data.recipes = {}
	potion.brew_recipes[typename] = data
end
...
local function register_recipe(typename, data)
	...
	potion.brew_recipes[typename].recipes[index] = recipe
end
...
function potion.get_brew_recipe(typename, items)
	...
	local recipe = potion.brew_recipes[typename].recipes[index]
	if recipe then
		local new_input = {}
		for i, stack in ipairs(items) do
...
potion.register_recipe_type("brew", {
	description = "Brewing",
	input_size = 2,
})

function potion.register_brew_recipe(data)
	data.time = data.time or 60
	potion.register_recipe("brew", data)
end

local recipes = {
--Base Potion
	{"lottfarming:berries 5",        "lottmystics:drinking_glass_water",         "lottmystics:wine"},
     {"farming:wheat 3",        "lottmystics:drinking_glass_water",         "lottmystics:beer"},
     {"lottplants:honey 6",        "lottmystics:drinking_glass_water",         "lottmystics:mead"},
     {"default:apple 5",        "lottmystics:drinking_glass_water",         "lottmystics:cider"},
     {"lottfarming:barley 6",        "lottmystics:drinking_glass_water",         "lottmystics:ale"},
}

for _, data in pairs(recipes) do
	potion.register_brew_recipe({input = {data[1], data[2]}, output = data[3], time = data[4]})
end
...
minetest.register_node("lottmystics:brewer", {
	description = machine_name,
	drawtype = "nodebox",
	tiles = {"default_wood.png"},
	node_box = {
        type = "fixed",
            fixed = potion.make_pipe( { {f=0.9,h1=-0.2,h2=0.2,b=0}, {f=0.75,h1=-0.50,h2=-0.35,b=0}, {f=0.75,h1=0.35,h2=0.5,b=0},
...
minetest.register_node("lottmystics:brewer_active", {
	description = machine_name,
	drawtype = "nodebox",
     tiles = {"default_wood.png"},
	node_box = {
...
minetest.register_abm({
	nodenames = {"lottmystics:brewer", "lottmystics:brewer_active"},
	interval = 1,
	chance = 1,
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		local result = potion.get_brew_recipe("brew", inv:get_list("src"))
		...
		if meta:get_float("fuel_time") < meta:get_float("fuel_totaltime") then
			...
			potion.swap_node(pos, "lottmystics:brewer_active")
			meta:set_string("formspec",
					...
					"list[current_name;dst;6,2;1,1;]"..
					"list[current_player;main;0,5;8,4;]"..
					"background[-0.5,-0.65;9,10.35;brewerbg.png]"..
					"listcolors[#606060AA;#888;#141318;#30434C;#FFF]")
			return
		end

		local recipe = potion.get_brew_recipe("brew", inv:get_list("src"))

		if not recipe then
			if was_active then
				meta:set_string("infotext", ("%s is empty"):format(machine_name))
				potion.swap_node(pos, "lottmystics:brewer")
				meta:set_string("formspec", formspec)
			end
			return
		end
